...
void ABlasterCharacter::RotateInPlace(float DeltaTime)
{
	...
	if (GetLocalRole() > ENetRole::ROLE_SimulatedProxy && IsLocallyControlled()) //enums are heirachical so autonomous and authority are ABOVE
	{
		AimOffset(DeltaTime);
	}
	else
	{
		TimeSinceLastMovementReplication += DeltaTime; //make sure this is called relatively often even if proxy isn't necessarily moving
		if (TimeSinceLastMovementReplication > 0.25f)
		{
			OnRep_ReplicatedMovement();
		}
		CalculateAO_Pitch(); //Called every frame
	}
}
...
void ABlasterCharacter::ReceiveDamage(AActor* DamagedActor, float Damage, const UDamageType* DamageType, AController* IntigatorController, AActor* DamageCauser)
{
	...
	if (Shield > 0.f)
	{
		//Shield Can absorb whole damage
		if (Shield >= Damage)
		{
			Shield = FMath::Clamp(Shield - Damage, 0.f, MaxShield);
			DamageToHealth = 0.f;
		}
		//Shield cannot absorb the whole damage so we subtract Existing shield from DamagetoHealth
		else
		{
	...
	if (Health == 0.f) {
		if (BlasterGameMode)
		{
			...
			ABlasterPlayerController* AttackerController = Cast<ABlasterPlayerController>(IntigatorController);
			BlasterGameMode->PlayerEliminated(this, BlasterPlayerController, AttackerController);
		}
	}
}
/*
* Called only when movement changes. This is useful for smoothing movement of simulated proxies because we can take the delta between each frame
* because this way the tick rate will not be faster than the net rate which causes the value of our simulated proxy to be zeroed out
* 
* It is zeroed because the tick is faster and thus the delta between some of the frames not getting net data has to be zero, which is not what we 
* want, BELOW IS THE FIX (Update on variable change (replication))
*/
...
{
	UE_LOG(LogTemp, Warning, TEXT("In Drop or destroy weapon"));
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("Should Drop"));
		Weapon->Dropped();
	}
}
...
void ABlasterCharacter::AimOffset(float DeltaTime)
{
	...
	if (Speed == 0.f && !bIsInair) // standing still, not jumping
	{
		...
		FRotator DeltaAimRotation = UKismetMathLibrary::NormalizedDeltaRotator(CurrentAimRotation, StartingAimRotation); //Difference in rotation from stopped position
	...
	if (Speed > 0.f || bIsInair) //Runnign or jumping
	{
...
void ABlasterCharacter::CalculateAO_Pitch()
{
	...
	if (AO_Pitch > 90.f && !IsLocallyControlled())
	{
		...
		//Take the newly uncompressed AO_pitch and compare it to the ranges and make a correction
		AO_Pitch = FMath::GetMappedRangeValueClamped(InRange, Outrange, AO_Pitch);
	}
}

/*
* This Function is necessary because we rotate the root bone for turning in place animation
* This method will not work for simulated proxies because this funtion is not updated every frame leading to a "jitter" 
* when other machines look at non locally controlled characters
* 
* Intead we should rotate the character and not the root bone
*/
void ABlasterCharacter::SimProxiesTurn()
{
	...
	if (Speed > 0.f)
	{
		//so that if we move we don't play rotate animation and slide
		TurningInPlace = ETurningInPlace::ETIP_NotTurning;
		return;
	}
	...
	if (FMath::Abs(ProxyYaw) > TurnThreshold)
	{
		...
		return; //return to skip below logic if the threshold was not met
	}
	TurningInPlace = ETurningInPlace::ETIP_NotTurning;
}
...
void ABlasterCharacter::SpawnDefaultWeapon()
{
	...
	if (BlasterGameMode && World && !bElimmed && DefaultWeaponClass)
	{
		...
		//Destroy default weapon so we don't have too many of them as the game progresses
