...
void ABlasterPlayerController::CheckPing(float DeltaTime)
{
	...
	if (HighPingRunningTime > CheckPingFrequency)
	{
		...
		{
			/*
			* Multiply by 4 because unreal compresses the data to get a uint8 -- this will get us an approximation good
			* enough.
			*
			...
			*/
			...
			if (PlayerState->GetPing() * 4 > HighPingThreshold)
			{
				...
				ServerReportPingStatus(true); //Server RPC to tell ping status
			}
			//Ping is not too high so false
			else 
			{
...
/*
* Server RPC to describe PingStatus
* 
* bHighPing true if ping is too high for server side rewind, false otherwise
*/
...
/*
* Sometimes the character overlay isn't initialized yet so we should poll the HUD variables once it is
*/
	...
	if (BlasterHUD)
	{
		...
		if (bHUDValid)
		{
			...
			if (BlasterGameState && BlasterPlayerState)
			{
				...
				//Player character won
				else if (TopPlayers.Num() == 1 && TopPlayers[0] == BlasterPlayerState)
				{
					InfoTextString = FString("You are the winner!");
				}
				//Someone else won, print their name
